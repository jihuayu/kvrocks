...
Status CheckNamespaceNonGetSubcommandState(Server *srv) {
  Config *config = srv->GetConfig();
  if (config->repl_namespace_enabled && config->IsSlave()) {
...
class CommandNamespaceSet : public Commander {
 public:
  Status Execute([[maybe_unused]] engine::Context &ctx, Server *srv, Connection *conn, std::string *output) override {
    if (auto s = CheckNamespaceNonGetSubcommandState(srv); !s.IsOK()) {
      return s;
    }
